...
describe('A11y Plugin', () => {
  ...
  it('should apply correct ARIA attributes to carousel elements', () => {
    ...
    slides.forEach((slide, index) => {
      ...
      expect(slide.getAttribute('aria-label')).toBe(`Slide ${index + 1} of ${slides.length}`);
  ...
  it('should use custom label when provided', () => {
    embla = EmblaCarousel(container, {}, [A11y({ carouselLabel: 'Test Carousel' })]);
  ...
  it('should use custom announcement template when provided', () => {
    embla = EmblaCarousel(
      container, 
      {}, 
      [A11y({ slideAnnouncementTemplate: 'Page {{currentSlide}} of {{totalSlides}}' })]
    );
  ...
  it('should disable announcements when announceSlideChanges is false', () => {
    ...
    embla = EmblaCarousel(container, {}, [A11y({ announceSlideChanges: false })]);
    const a11yPlugin = embla.plugins().a11y;
  ...
  it('should warn in development mode when no accessible names are provided', () => {
    ...
    expect(console.warn).toHaveBeenCalledWith(expect.stringContaining('No accessible name provided for carousel'));
    expect(console.warn).toHaveBeenCalledWith(expect.stringContaining('Slides should have descriptive labels'));
  });
  
  it('should not show warnings when showDeveloperWarnings is false', () => {
    embla = EmblaCarousel(container, {}, [A11y({ showDeveloperWarnings: false })]);
  ...
  it('should not show warnings when appropriate labels are provided', () => {
    ...
    expect(console.warn).not.toHaveBeenCalledWith(expect.stringContaining('No accessible name provided for carousel'));
    expect(console.warn).not.toHaveBeenCalledWith(expect.stringContaining('Slides should have descriptive labels'));
    expect(console.warn).toHaveBeenCalledWith(expect.stringContaining('navigation buttons'));
  });
});